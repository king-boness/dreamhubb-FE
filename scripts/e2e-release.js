...
/**
 * E2E Release - Builds FE, starts FE prod server + BE, waits for readiness, runs E2E tests, then cleans up
 *
 * Usage:
 *   npm run e2e:release
 *
 ...
 *   E2E_BASE_URL - Frontend URL (default: http://localhost:9001)
...
const FE_PORT = process.env.E2E_BASE_URL?.replace("http://localhost:", "").replace("/", "") || "9001";
...
function log(message) {
  console.log(`[e2e:release] ${message}`);
}
...
function startFrontendProd() {
  ...
  if (!fs.existsSync(distPath)) {
    throw new Error(`Frontend build not found at ${distPath}. Run 'npm run build' first.`);
  }

  // Use a simple HTTP server (e.g., serve or http-server)
  // For simplicity, we'll use npx serve
  const feProcess = spawn("npx", ["-y", "serve", "-s", distPath, "-l", FE_PORT], {
    cwd: path.join(__dirname, ".."),
    stdio: "inherit",
    shell: true
  });
...
function runE2ETests() {
  log("Running E2E tests against production build...");
  return new Promise((resolve, reject) => {
      ...
      new Promise((resolve) => {
        try {
          if (process.platform === "win32") {
            ...
            setTimeout(() => {
              try {
                process.kill(-feProcess.pid, "SIGKILL");
              } catch {
      ...
      new Promise((resolve) => {
        try {
          if (process.platform === "win32") {
            ...
            setTimeout(() => {
              try {
                process.kill(-beProcess.pid, "SIGKILL");
              } catch {
                // Process already terminated
              }
              resolve();
